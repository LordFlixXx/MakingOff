...
const getFilters = () => {
  ...
  const minimumRating = document.querySelector(
    ".filter-dropdowns select.rating-filter"
  ).value;
  const sortBy = document.querySelector(
    ".filter-dropdowns select.sort-by-filter"
  ).value;
  const orderBy = document.querySelector(
    ".filter-dropdowns select.order-by-filter"
  ).value;
...
document.querySelector(".filter-bar").addEventListener("change", applyFilters);
